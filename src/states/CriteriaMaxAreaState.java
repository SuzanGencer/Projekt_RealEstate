package states;

import context.Context;
import java.util.Arrays;

/**
 * @author Gencer on
 * @project RealEstate
 **/

public class CriteriaMaxAreaState extends State {
	{
		setStateOptions(Arrays.asList("Please write max Area"));
	}

	@Override
	public void enterKey1(Context context) {
		getMaxArea(context);
	}

	@Override
	public void enterKey2(Context context) {
		getMaxArea(context);
	}

	@Override
	public void enterKey3(Context context) {
		getMaxArea(context);
	}

	@Override
	public void enterKey4(Context context) {
		getMaxArea(context);
	}

	@Override
	public void enterKey5(Context context) {
		getMaxArea(context);
	}

	@Override
	public void enterKey6(Context context) {
		getMaxArea(context);
	}

	@Override
	public void enterKey7(Context context) {
		getMaxArea(context);
	}

	@Override
	public void enterKey8(Context context) {
		getMaxArea(context);
	}

	@Override
	public void enterYes(Context context) {
		staySameState();
	}

	@Override
	public void enterOtherKeys(Context context) {
		getMaxArea(context);
	}

	private void getMaxArea(Context context) {
		try {
			realEstate.setActivSearch(realEstate.getAllHousesWithMaxArea(Double.parseDouble(context.getLine())));
			context.setActiveState(new ResultOrAddCriteriaState());
		} catch (NumberFormatException e) {
			staySameState();
		}
	}
}