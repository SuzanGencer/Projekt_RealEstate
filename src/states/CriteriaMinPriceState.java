package states;

import context.Context;
import java.util.Arrays;

/**
 * @author Gencer on
 * @project RealEstate
 **/

public class CriteriaMinPriceState extends State {
	{
		setStateOptions(Arrays.asList("Please write min Price"));
	}

	@Override
	public void enterKey1(Context context) {
		getMinPrice(context);
	}

	@Override
	public void enterKey2(Context context) {
		getMinPrice(context);
	}

	@Override
	public void enterKey3(Context context) {
		getMinPrice(context);
	}

	@Override
	public void enterKey4(Context context) {
		getMinPrice(context);
	}

	@Override
	public void enterKey5(Context context) {
		getMinPrice(context);
	}

	@Override
	public void enterKey6(Context context) {
		getMinPrice(context);
	}

	@Override
	public void enterKey7(Context context) {
		getMinPrice(context);
	}

	@Override
	public void enterKey8(Context context) {
		getMinPrice(context);
	}

	@Override
	public void enterYes(Context context) {
		staySameState();
	}

	@Override
	public void enterOtherKeys(Context context) {
		getMinPrice(context);
	}

	private void getMinPrice(Context context) {
		try {
			realEstate.setActivSearch(realEstate.getAllHousesWithMinPrice(Double.parseDouble(context.getLine())));
			context.setActiveState(new ResultOrAddCriteriaState());
		} catch (NumberFormatException e) {
			staySameState();
		}
	}
}